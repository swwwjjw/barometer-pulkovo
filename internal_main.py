...
def get_stats(role_index: int, filter_outliers: bool = True):
    ...
    # Pulkovo vs Market
    # Use fixed pulkovo_salary from config if available, otherwise calculate from data
    if "pulkovo_salary" in role_config:
        pulkovo_avg = float(role_config["pulkovo_salary"])
    else:
...
@app.get("/api/b1/positions/{block_index}/{position_index}")
def get_b1_position(block_index: int, position_index: int):
    """Get detailed information about a specific position"""
    if block_index < 0 or block_index >= len(B1_DATA):
        raise HTTPException(status_code=404, detail="Block not found")
    
    block = B1_DATA[block_index]
    if position_index < 0 or position_index >= len(block["positions"]):
        raise HTTPException(status_code=404, detail="Position not found")
    
    return block["positions"][position_index]
